...
function experimentInit() {
  ...
  image_stimulus = new visual.ImageStim({
    ...
    image : undefined, mask : 'circle',
    ori : 0, pos : [0, 0], size : [0.5, 0.5],
    color : new util.Color([1, 1, 1]), opacity : 1,
    flipHoriz : false, flipVert : false,
    texRes : 128, interpolate : true, depth : 0.0 
  });
  circle_border = new visual.Polygon ({
    win: psychoJS.window, name: 'circle_border', 
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    circle_border.setLineColor(new util.Color(color));
    arrow_image.setImage(os.path.join('stimuli', 'arrows', arrow));
    stop_signal_sound.secs=0.25;
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    if (stop_signal_sound.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      if (0.25 > 0.5) {  stop_signal_sound.stop();  // stop the sound (if longer than duration)
